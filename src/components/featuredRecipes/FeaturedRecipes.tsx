...
async function fetchRecipes(): Promise<IRecipe[]> {
  ...
  // Randomize recipes server-side
  return result.map((card: { recipeTitles: string[]; ids:number[]; date:string[]; minutes:string[]; 
    recipeImages:string[]; descriptions:string[]; detailDescriptions:string[]; }) => {
    const recipeCount = card.recipeTitles.length;
