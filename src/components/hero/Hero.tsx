...
// yeh function data fetch krta hay from sanity
...
function Hero() {
  ...
  //use effect har haal mein chalayga jab jab component render karay ga..yeh use effect herodata ko fetch karta hay or store krta hay use state k variable mein set karta hay
  useEffect(() => {
    const getData = async () => {
  ...
  const { heroImage, para1, mainHeading } = heroData;
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center px-4 xsm:px-6 sm:px-8 md:px-12">
        ...
        <Link href="/blog">
          <button
            ...
            className="inline-flex items-center bg-[#7C4EE4] text-white border-0 py-2 px-4 xsm:py-2 xsm:px-5 focus:outline-none hover:underline hover:scale-110 transition-all duration-300 rounded text-sm xsm:text-base md:mt-0 group"
          >
            Explore Blogs
